...
from threedp_accelerometer.octoprint.sampling import SamplingJob
...
class Args:
    def __init__(self) -> None:
        ...
        def xy_pos_from_str(pos: str) -> None | Tuple[int, int]:
            x, y = pos.strip("\"").split(",")
            x = int(x)
            y = int(y)
            return x, y
        ...
        default_filename = f"./op-capture-{timestamp}.tsv"
        ...
        mux_grp.add_argument(
            "-f", "--file",
            help=f"Specify otuput file (*.tsv). Leave empty for default filename.",
            type=str,
            nargs='?',
